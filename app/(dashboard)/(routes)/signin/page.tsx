...
const SignInPage: React.FC = () => {
  ...
  const onSubmit = async (data: SignInFormData) => {
    try {
      await SIGIN_WIRING.login(data.usernameOrEmail, data.password);
      toast.success('Sign-in successful! Welcome back to Darkdrop.');
      router.push('/dashboard');
    } catch (error: any) {
      toast.error(
        error.message ||
          'Please enter valid details. Contact admin at subhamsingh39621@gmail.com if issues persist.'
      );
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            ...
            {errors.usernameOrEmail && (
              <p id="usernameOrEmail-error" className="text-red-500 text-sm">
